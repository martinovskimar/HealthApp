...
public class HealthActivitiesClient {
	 ...
	 private static final String[] EXERCISE_TYPES = {"Running", "Swimming", "Cycling", "Weightlifting", "Yoga", "Pilates"};
	    ...
	    public HealthActivitiesClient(String serviceName) {
	        // Discover service with jmDNS
	        ...
	        try {
	            ...
	            System.out.printf("Discovered service at %s:%d%n", serviceInfo.getHostAddresses()[0], serviceInfo.getPort());
	            channel = ManagedChannelBuilder.forAddress(serviceInfo.getHostAddresses()[0], serviceInfo.getPort())
	                    .usePlaintext()
	                    .build();
	        } catch (Exception e) {
	            e.printStackTrace();
	            throw new RuntimeException("Error discovering service", e);
	        }
	    ...
	    public static void main(String[] args) throws Exception {
	    	HealthActivitiesClient client = new HealthActivitiesClient("HealthActivitiesService");
            client.blockingStub.exercise(ExerciseRequest.newBuilder()
                    .setExerciseType("Running")
                    .build());
	    	
//	    	NutritionClient client = new NutritionClient("HealthActivitiesService");
//	    	
